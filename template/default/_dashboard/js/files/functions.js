$(function() {
    ...
    $(".removeButton").on("click", function() {
        ...
        $('#userConfirmDialog').data("user_id", userId).dialog('open');
        return false;
    });

    $("#userConfirmDialog").dialog({
        autoOpen: false,
        width: 400,
        buttons: {
            "Да": function () {
                ...
                $.post('/index.php?module=user&action=delete&dashboard=1&is_ajax=1', {"id": userId}, function(response) {
                    if (response.success) {
                        $.jGrowl("Пользователь удален", {
                            header: 'Успешно!',
                            position: 'bottom-right'
                        });

                        $("#user_" + userId).slideUp("slow");
                    }
                    if (response.error) {
    ...
    tinymce.init({
        selector: "textarea#StaticPageContentTextarea",
        plugins: [
