...
{block name="content"}
    ...
    <div class="content page-messages" id="pm_inbox_block">
        <h2>{"Входящие сообщения"|gettext}</h2>
            ...
            <tbody>
                {foreach $inbox_messages as $message}
                    <tr id="msg_row_{$message.id}">
                        ...
                        <td>{$message.from_first_name} {$message.from_last_name}</td>
                        <td class="strong"><a href="/?module=user_pm_show&messageFolder=inbox&messageId={$message.id}">{$message.subject}</a></td>
